...
\begin{figure}[htbp]
        \centering
        \includegraphics[width=0.55\linewidth]{figures/electronicsignalschematic.png}
...
\begin{figure}[htbp]
    ...
    \caption{Fringe detection result obtained from the electronic photodiode signal. The modified amplitude-based evaluation allowed fringes to be detected despite baseline fluctuations and signal noise.}
    \label{fig:electronic_signal_detection}
\end{figure}

\section{Comparison of Detection Methods}

The two investigated detection approaches showed a different performance. The camera provided stable and clearly visible interference fringes with a high signal-to-noise ratio, enabling robust image-based evaluation. Furthermore, the recorded fringe patterns offered direct visual feedback during alignment and facilitated the development of automated analysis software.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/camera_diode_side_by_side.png}
...
The two phase-shifted photodiode signals were successfully detected, as shown in Figure~\ref{fig:homodyne_first_results}. The direction of motion can be determined from the Lissajous plot shown on the right-hand side of the figure. The two quadrature signals form a circular trajectory, and the direction in which this trajectory is moving indicates the direction of motion.

The measurement demonstrated the functionality of the quadrature homodyne extension. 

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/homodynefirstresult.jpg}
    \caption{Measurement result obtained with the quadrature homodyne interferometer. The two photodiode signals were detected successfully, and the Lissajous representation was used to evaluate the direction-dependent phase relation.}
    \label{fig:homodyne_first_results}
\end{figure}

A more detailed evaluation of the recorded quadrature signals is shown in Figure~\ref{fig:sin_cos_fit}. One photodiode signal was fitted with a sine function, while the second signal was fitted with a cosine function. The fitted curves show a phase shift of \(90^\circ\), as indicated by the dashed vertical reference line passing through corresponding points of the two fitted signals. This confirms that the optical setup generated the required quadrature phase relation for direction-sensitive displacement detection.
