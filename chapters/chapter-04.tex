...
The selection of the laser source completed the optical design of the measurement system. The subsequent chapter focuses on the software developed to evaluate the interferometric signals.
...
A screenshot of the final software is shown in Figure~\ref{fig:software_gui}.
...
These parameters were determined experimentally and were found to provide reliable fringe detection under the investigated operating conditions. A smaller offset was used for the dark threshold than for the bright threshold \(0.125\) compared with \(0.40\), because bright fringes were reached more clearly than dark fringes in the recorded camera signal.
...
After a successful detection, a short cooldown period is applied before another fringe can be registered. This prevents multiple counts of the same fringe caused by small oscillations or electrical noise. Furthermore, requiring several consecutive samples in both the dark and bright states significantly reduces false detections.

The control of the translation stage as well as distance calculation and all further calculations are identical to the Camera Software mentioned above.

Finally, the software provides the possibility to record all measured data. The recorded dataset contains the relative measurement time, the raw voltage, the baseline-corrected voltage, the accumulated fringe count, the calculated optical distance and the current position of the translation stage. After the measurement, these data can be exported as a CSV file for further analysis. A visualization of the User Interface can be seen in this figure ...
...
The fringe count is then detected using only one of the photodiode and the exact same fringe counter as in the Single Photo diode software.

\subsection{Phase Determination}

The phase difference between the two normalized signals is calculated directly using the four-quadrant arctangent,
