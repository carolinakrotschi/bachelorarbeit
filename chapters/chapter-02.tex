...
where \(z_R\) is the Rayleigh range (defined in Section~\ref{sec:beam_waist_divergence}), the Gaussian beam solution can be written as
...
where \(A_0\) is the field amplitude at the beam waist, \(w_0\) is the beam waist radius, \(w(z)\) is the beam radius at the position \(z\), \(R(z)\) is the radius of curvature of the wavefront, and \(\zeta(z)=\arctan\!\left(\frac{z}{z_R}\right)\) \prismcite{8}.
...
\subsection{Beam Waist and Divergence}\label{sec:beam_waist_divergence}
...
where \(z_R\) is the Rayleigh length, defined as the distance from the beam waist at which the beam radius has increased by a factor of \(\sqrt{2}\) \prismcite{8}.
...
This formalism was then generalized by Anthony Siegman for arbitrary real laser beams by introducing beam widths based on the statistical standard deviation of the intensity distribution:
...
For completeness, the main optical components used in the quadrature homodyne interferometer are introduced in the following sections. The \(\lambda/2\) wave plate, \(\lambda/4\) wave plate, and Wollaston prism form the core components and are discussed in detail, while the polarizing beam splitter is covered briefly at the end.
...
A polarizing beam splitter is also used in the setup to separate an incoming beam into two orthogonal linear polarization components. 
