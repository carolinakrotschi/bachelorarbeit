...
\begin{equation}
\Delta\phi=\operatorname{atan2}(S_2,S_1),
\end{equation}

which uniquely determines the phase over the full $2\pi$ range \prismcite{32}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.55\linewidth]{figures/countinghomodyne.png}
    \caption{Principle of direction-sensitive quadrature signal evaluation in homodyne interferometry. The phase relation between two quadrature signals enables the direction of motion to be determined. Obtained from \prismcite{32}.}
...
Specifically, a $\lambda/2$ wave plate, a $\lambda/4$ wave plate, a Wollaston prism, and a wire-grid polarizer. The basic functions of a conventional beam splitter and a polarizing beam splitter are only mentioned briefly here. A conventional beam splitter divides the incoming beam into two beams of similar intensity, while a polarizing beam splitter separates the beam into two orthogonal polarization components. The Wollaston prism is discussed in more detail because it is a special type of polarizing beam splitter.

\paragraph{Half-Wave Plate}

A half-wave plate is a special type of wave retarder. In general, a wave retarder is a birefringent optical element with two principal axes, usually referred to as the fast and slow axes. Because the refractive indices along these axes are different, the two orthogonal polarization components of a beam propagate with different phase velocities. As a result, a phase delay is introduced between the two components \prismcite{35}.
...
\begin{figure}[htbp]
    ...
    \caption{Schematic representation of a Wollaston prism. The two cemented birefringent prisms have perpendicular optical axes, causing orthogonally polarized components to be refracted in different directions. Obtained from \prismcite{37}.}
    \label{fig:wollaston_prism}
\end{figure}

\paragraph{Wire-Grid Polarizer}

A wire-grid polarizer (shown in Figure~\ref{fig:wire_grid_polarizer}) consists of many parallel conducting wires. When light reaches the grid, its electric field can be decomposed into a component parallel to the wires and a component perpendicular to the wires \prismcite{39}. The electric field component parallel to the wires drives the free electrons in the metal along the wire direction. The resulting oscillating currents reradiate electromagnetic waves, leading mainly to reflection and absorption of this polarization component.

For the electric field component perpendicular to the wires, the electrons cannot move efficiently across the gaps between neighboring wires. Consequently, only a weak current is induced, and this polarization component is transmitted much more efficiently. The wire-grid polarizer therefore transmits light polarized perpendicular to the wires while suppressing the parallel component.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.55\linewidth]{figures/wiregrid.png}
    \caption{Principle of a wire-grid polarizer. The electric field component parallel to the conducting wires is mostly reflected or absorbed, while the perpendicular component is transmitted. Obtained from \prismcite{39}.}
