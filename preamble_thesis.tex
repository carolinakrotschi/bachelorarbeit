...
\usepackage[twoside,bindingoffset=0.7cm,left=2cm,right=2cm,top=2cm,bottom=3cm]{geometry}        %   for page/geometry layout -- https://www.ctan.org/pkg/geometry
...
\usepackage[backend=biber,style=numeric-comp,sorting=none]{biblatex}  %   for using bibliography, references and citations -- https://ctan.org/pkg/biblatex
...
\newcommand*{\getAuthor}{Carolina Krotsch}
\newcommand*{\getSupervisorOne}{Prof. Dr. Ferenc Krausz}
\newcommand*{\getSupervisorTwo}{Dr. Nicholas Karpowicz}
\newcommand*{\getSupervisorThree}{Dr. Sergio Revuelta}
% \newcommand*{\getExamDate}{Date of final exam}
% -- english -- %
\newcommand{\getTitleEN}{Michelson Interferometer for Correction of Translation Stage Deviation}
\newcommand{\getSubtitleEN}{Maybe with some Subtitle}
\newcommand*{\getPrintLocationEN}{Munich}
\newcommand*{\getPrintYearEN}{\the\year}
\newcommand*{\getPlaceOfBirthEN}{Munich}
\newcommand*{\getSubmissionDateEN}{28.07.2026}
\newcommand*{\langEN}{en-US}
% -- german -- %
\newcommand{\getTitleDE}{Michelson-Interferometer zur Korrektur von Abweichungen eines Translationstisches}
\newcommand{\getSubtitleDE}{Vielleicht mit Untertitel}
\newcommand*{\getPrintLocationDE}{München} 
\newcommand*{\getPrintYearDE}{\the\year}
\newcommand*{\getPlaceOfBirthDE}{München}
\newcommand*{\getSubmissionDateDE}{28.07.2026}
%\newcommand*{\langDE}{de}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%% BIBLATEX SETUP %%%
% Suppress the bib "language" field (e.g. "en"/"de") from being printed in the bibliography:
% every entry driver in standard.bbx calls \printlist{language} directly (not via a
% bibmacro), and \DeclareListFormat{language}{...} controls how that list is rendered,
% so overriding it here to produce no output is the correct hook.
\DeclareListFormat{language}{}
\makeatletter
\newif\if@prismcitefirst
\DeclareRobustCommand{\prismcite}[1]{[%
  \@prismcitefirsttrue
  \@for\nextgp:=#1\do{%
    \if@prismcitefirst
      \@prismcitefirstfalse
